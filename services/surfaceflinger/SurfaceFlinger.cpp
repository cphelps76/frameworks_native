...
void SurfaceFlinger::doComposeSurfaces(const sp<const DisplayDevice>& hw, const Region& dirty)
{
    ...
    if( lastOrientation != mCurrentState.orientation)
    {
        lastOrientation = mCurrentState.orientation;
        return;
    }
    ...
    int request=0;
    int ret;
    int holex=0,holey=0,holew=0,holeh=0,holelayer=0;
    char buf[SYSCMD_BUFSIZE] = {0};
#ifdef ENABLE_SCALE_FUNCTION	
    ...
    else
    {
        ...
        if(sscanf(buf, "%i", &request)==1)
        {
            if(request>0)
            {
                sscanf(buf, "%i %i ", &request,&request_type);
    ...
    const int32_t id = hw->getHwcDisplayId();
    HWComposer& hwc(getHwComposer());
    if((request == 7)||((Last2XScaleValue==7)&&(request==0)))
    {
        char scalest[8];
        float scale_x=0.0f;
        float scale_y=0.0f;
        ALOGE("request 2x scale request_width=%d request_height=%d\n",request_width,request_height);
        ...
        {
            scale_x= (float)960/windowWidth;
            scale_y= (float)1080/windowHeight;
        }
    ...
    else if((request == 8)||((Last2XScaleValue==8)&&(request==0)))
    {
        ...
        { 
            changeBufferTo3DFormat(scale_x,scale_y,request_type);
        }
        if(iptv != 8 && iptv !=7)
            drawVideoHole( holex*scale_x,holey*scale_y,holew*scale_x,holeh*scale_y );
    }
    ...
    else if((request == 17)||((Last2XScaleValue==17)&&(request==0)))
    {
        changeBufferTo3DFormat(1,1,1);
    }
    else if((request == 18)||((Last2XScaleValue==18)&&(request==0)))
    {
        changeBufferTo3DFormat(1,1,2);
    }
    else{
        HWComposer::LayerListIterator cur = hwc.begin(id);
        const HWComposer::LayerListIterator end = hwc.end(id);
        ...
        if (hasGlesComposition) {
            ...
            glLoadIdentity();
            glTranslatef((float)hw->getWidth()-windowWidth , (float)hw->getHeight()-windowHeight,0);
			...
			glTranslatef((float)windowWidth-hw->getWidth(), (float)windowHeight-hw->getHeight(),0);
        }
        ...
        if (cur != end) {
            ...
            for (size_t i=0 ; i<count && cur!=end ; ++i, ++cur) {
                ...
                {	
                    ...
                    if(!strcmp(property, "true")){ //if prop is "true", draw the hole
                        while (it != end) {
                            const Rect& r = *it;
                            ALOGW("clip %i %i %i %i ",r.left,r.top,r.right,r.bottom);
                            drawVideoHole( r.left,r.top,r.right-r.left-1,r.bottom-r.top-1);
                            it++;
                        }
						continue;
                    }
                ...
                if((holeh>0 && holew>0) && i==holelayer  )
                {
                    drawVideoHole( holex*1.0f,holey*1.0f,holew*1.0f,holeh*1.0f );
                }
            }
        } else {
        ...
        const Region& region) const
{
    ...
    const int32_t height = windowHeight;    //hw->getHeight()
...
void SurfaceFlinger::doTranslateOffsetInRender(int offsetx,int offsety,bool flag)
{
		
    DisplayDeviceState& disp(mCurrentState.displays.editValueAt(0)); 
	//(disp.orientation - hw.default.oriention+4)%4
	int orientation  =(disp.orientation-3+4)%4;
    ...
    if(windowSize)
    {
        if(orientation==0)              //not test.
        {
            gloffsetx=0;
            gloffsety=0;
        }else if(orientation==1)        //only test this .
        {
            gloffsetx=0;
            gloffsety=(float)-offsety;
        }else if(orientation==2)
        {
            gloffsetx=(float)-offsetx;
            gloffsety=(float)-offsety;
    ...
    if(flag)
    {
        glTranslatef(gloffsetx,gloffsety,0);
    }else
    {
        glTranslatef(-gloffsetx,-gloffsety,0);
    }
...
// ---------------------------------------------------------------------------
void  SurfaceFlinger:: doTranslateOffset(int offsetx,int offsety, bool flag)
{
	//here maybe problem at 0.
    DisplayDeviceState& disp(mCurrentState.displays.editValueAt(0)); 
        ...
        uint32_t minLayerZ, uint32_t maxLayerZ)
{
    ...
    const uint32_t hw_h = hw->getHeight();
    if(!windowSize){
        windowWidth = hw->getWidth();
        windowHeight = hw->getHeight();
    }
